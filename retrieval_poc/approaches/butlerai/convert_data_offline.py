...
def main():
    ...
    for idx, row in df.iterrows():
        rule = row['Rule']
        print(f"Converting {idx + 1}/{len(df)}: {rule}")
        
        # Process both Example 1 and Example 2
        for example_num in [1, 2]:
            example_col = f'Example {example_num}'
            if example_col in row and pd.notna(row[example_col]):
                query = row[example_col]
                ...
                if json_match:
                    ...
                    new_row['example_number'] = example_num
                    converted_data.append(new_row)
                    
                    print(
                        f"  ✅ Example {example_num} converted: "
                        f"{natural_language[:80]}..."
                    )
                else:
                    ...
                    converted_data.append(new_row)
                    print(
                        f"  ⚠️  Example {example_num} no JSON found, "
                        f"keeping original"
                    )
